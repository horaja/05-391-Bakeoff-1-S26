...
var numberOfSquaresWide = 4;
var numberOfSquaresTall = 4;
var canvasSize = 700;
// Trial configuration
var reportURL = "https://bakeoff-1-data.cmu-dhcs.workers.dev/";
var competitionNumberOfTasks = 10;
...
var MiniView = /** @class */ (function () {
    function MiniView(canvasElement, squares) {
        // consts to change the size/layout of the display
        this.margin = 0; //set the margin around the squares
        this.padding = 2; // padding between buttons and also their width/height
        this.squareSize = 25; // padding between buttons and also their width/height
        this.width = numberOfSquaresWide * (this.squareSize + this.padding) - this.padding + this.margin; // overall view width
        this.height = numberOfSquaresTall * (this.squareSize + this.padding) - this.padding + this.margin; // overall view height
    ...
    MiniView.prototype.clear = function () {
        // clear the screen
        this.draw.clearRect(0, 0, this.width, this.height);
        // draw 'em all in grey
        for (var i = 0; i < this.squares.length; i++) {
            for (var j = 0; j < squares[i].length; j++) {
                var x = (j * (this.padding + this.squareSize)) + this.margin;
                var y = (i * (this.padding + this.squareSize)) + this.margin;
                this.draw.fillStyle = "#ccc";
                this.draw.fillRect(x, y, this.squareSize, this.squareSize);
            }
        }
    };
    MiniView.prototype.render = function (highlightedID) {
        // clear the screen
        this.draw.clearRect(0, 0, this.width, this.height);
...
var Trial = /** @class */ (function () {
    function Trial(numberOfTasks, teamName, verbose) {
        var _this = this;
        this.squares = squares;
        ...
        this.trialButton.onclick = function () {
            _this.toggle();
        };
        this.realTrialIndicator = document.getElementById("realTrial");
        this.renderer = new MiniView(document.getElementById("trial_display"), this.squares);
        // default events
        this.addEventListener("testOver", function () { console.log("Test over. Splits:", _this.splits, "; wrong clicks: ", _this.wrongClicks); });
        this.addEventListener("wrongSquare", function () { console.log("Wrong square clicked."); });
        this.addEventListener("correctSquare", function () { console.log("Correct square clicked in:", _this.splits.slice(-1)[0]); });
        this.reset();
    }
    // what happens when the start/stop button is pressed
    Trial.prototype.toggle = function () {
        if (this.live == false) {
            this.start();
        }
        else
    ...
    Trial.prototype.reset = function () {
        var possibleTasks = [];
        for (var i = 1; i <= numberOfSquaresTall * numberOfSquaresWide; i++) {
    ...
    Trial.prototype.nextTask = function () {
        ...
        if (this.currentTaskNumber < this.tasks.length) { // if there's at least one task left
            ...
            if (this.realTrialIndicator.checked == true) {
                var data = { splits: this.splits, wrongClicks: this.wrongClicks, teamName: this.teamName };
    ...
    Trial.prototype.call = function (eventName) {
        if (this.verbose)
    ...
    */
    Trial.prototype.getSquaresData = function () {
        return this.squares;
    };
    /*
    ...
    */
    Trial.prototype.submitClick = function (index) {
        if (this.verbose)
    ...
    */
    Trial.prototype.addEventListener = function (eventName, callback) {
