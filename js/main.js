...
function makeSquaresUsingCanvasAPI(trial) {
    ...
    var margin = 200; // the margin around the squares
    var padding = 50; // padding between squares
    var squareSize = 40; // their width/height
    ...
    for (var rowNumber = 0; rowNumber < squares.length; rowNumber++) {
        for (var columnNumber = 0; columnNumber < squares[rowNumber].length; columnNumber++) {
            ...
            var x = (columnNumber * (padding + squareSize)) + margin;
            var y = (rowNumber * (padding + squareSize)) + margin;
            // get the id and color data for this square
            var squareID = squares[rowNumber][columnNumber].id;
            var squareColor = squares[rowNumber][columnNumber].color;
    ...
    canvasElement.addEventListener("click", function (e) {
        ...
        var rowNumber = Math.floor((y - margin) / (padding + squareSize));
        var columnNumber = Math.floor((x - margin) / (padding + squareSize));
...
function makeSquaresUsingSVG(trial) {
    ...
    var svg = SVG().addTo('#main').size(canvasSize, canvasSize);
    // As before -- a nested loop to go through the provided squares data.
    for (var rowNumber = 0; rowNumber < squares.length; rowNumber++) {
        var _loop_2 = function (columnNumber) {
            // Calculate where this square should be:
            var x = (columnNumber) * (padding + squareSize) + margin;
            var y = Math.floor(rowNumber) * (padding + squareSize) + margin;
            // get the id and color data for this square
            var squareID = squares[rowNumber][columnNumber].id;
            var squareColor = squares[rowNumber][columnNumber].color;
            ...
            var square = svg.rect(squareSize, squareSize);
